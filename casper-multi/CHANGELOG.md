...
### Added
- Language filtering on author archives now matches the existing tag archive
  behavior. Author post cards expose all public and internal tag names, and
  the selected locale from `localStorage` determines which cards are shown.
- `window.casperMultiLanguageContext`, generated by
  `partials/hreflang-multi.hbs`, exposes the current content type, `#multi`
  state, detected language, and translated post URLs to the global language
  selector.
- `AUTHOR_LANGUAGE_CHANNEL_PROPOSAL.md`, documenting a deferred server-side
  alternative that uses language-specific Ghost channels for exact author
  pagination without modifying Ghost core.

### Changed
- The language selector now treats both tag and author archives as
  language-filtered views. Changing locale reloads the same archive URL so the
  client-side filter can re-run with the new preference.
- On a post tagged `#multi`, changing locale navigates directly to that post's
  translated `-en` or `-ko` counterpart. A post without `#multi`, pages, and
  all other non-archive screens continue to navigate to the selected locale's
  home page.
- Tag and author archives share `filterArchivePostsByLanguage()` from
  `assets/js/util.js`, keeping tag-order-independent `#en` / `#ko` detection
  consistent across both templates.

### Release
- Logical release name: `multi-0.0.9`. The inherited Casper
  `package.json` version remains `5.7.0` by project convention.
